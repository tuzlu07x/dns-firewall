...
class FilterManager {
    constructor(config) {
        this.denyListDomains = new Map();
        this.denyListRegex = new Map();
        this.allowListDomains = new Map();
        this.allowListRegex = new Map();
        this.config = config;
    }
    /**
        ...
        for (const [listName, sources] of Object.entries(this.config.denylists)) {
            ...
            for (const source of sources) {
                try {
                    ...
                    console.error(`Error loading deny list ${listName} from ${source}:`, error);
                }
            }
        ...
        for (const [listName, sources] of Object.entries(this.config.allowlists)) {
            ...
            for (const source of sources) {
                try {
                    ...
                    console.error(`Error loading allow list ${listName} from ${source}:`, error);
                }
            }
     ...
     */
    async checkDomain(domain, clientInfo) {
        if (!this.config.enabled) {
     ...
     * @param clientInfo Client information
     */
    async checkResponse(response, clientInfo) {
        if (!this.config.enabled ||
            !response.answers ||
     ...
     */
    isDomainMatchingRegex(domain, regexList) {
     ...
     */
    async loadFromFile(filePath, domains, regexPatterns) {
     ...
     * @param regexPatterns Array to add regex patterns to
     */
    async loadFromUrl(url, domains, regexPatterns) {
     ...
     * @param regexPatterns Array to add regex patterns to
     */
    parseDomainsFromContent(content, domains, regexPatterns) {
